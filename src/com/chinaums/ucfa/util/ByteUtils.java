...
public class ByteUtils {
	...
	public static final byte[] getIntegrationBytesWithLength(String info) {
		...
		try {
			...
			MyLog.d("getIntegrationBytesWithLength UnsupportedEncodingException = "
					+ e.toString());
		}
		byte[] data = null;
		int lengthData = byteData.length;
		...
		for (int i = 0; i < integrationBytes.length; i++) {
			if (strData[i] != null) {
				try {
					...
					MyLog.d("getIntegrationBytes UnsupportedEncodingException = "
							+ e.toString());
				}
		...
		for (int i = 0; i < integrationBytes.length; i++) {
			if (strData[i] != null) {
				try {
					...
					MyLog.d("getIntegrationBytes1 UnsupportedEncodingException = "
							+ e.toString());
				}
				lengthTemp = integrationBytes[i].length;
				lengthData += lengthTemp;
